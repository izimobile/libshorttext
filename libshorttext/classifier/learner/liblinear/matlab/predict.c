...
INT64 col_format_flag;

void read_sparse_instance(const mxArray *prhs, INT64 index, struct feature_node *x, INT64 feature_number, double bias)
{
    INT64 i, j, low, high;
    mwIndex *ir, *jc;
    double *samples;
    ...
    j = 0;
    low = (INT64) jc[index], high = (INT64) jc[index+1];
    for(i=low; i<high && (INT64) (ir[i])<feature_number; i++)
    {
        x[j].index = (INT64) ir[i]+1;
        x[j].value = samples[i];
        j++;
    }
    if(bias>=0)
...
void do_predict(mxArray *plhs[], const mxArray *prhs[], struct model *model_, const INT64 predict_probability_flag)
{
    INT64 label_vector_row_num, label_vector_col_num;
    INT64 feature_number, testing_instance_number;
    INT64 instance_index;
    ...
    testing_instance_number = (INT64) mxGetM(prhs[1]);
    if(col_format_flag)
    {
        feature_number = (INT64) mxGetM(prhs[1]);
        testing_instance_number = (INT64) mxGetN(prhs[1]);
    }

    label_vector_row_num = (INT64) mxGetM(prhs[0]);
    label_vector_col_num = (INT64) mxGetN(prhs[0]);
